...
// Above Code is from Claude and Below Code is from Google AI Studio.
...
function buildAllDispatches(jobs, inventoryItems) {
  ...
  for (const job of jobs) {
    list.push({
      ...
      amountPending: job.amountPending || 0,
    });
  }
...
export default function ConnectionJobsLog() {
  ...
  const connectionPendingTotal = connectionDispatches.reduce(
    (s, d) => s + d.amountPending,
    0,
  );
  const adhocTotal = adhocDispatches.reduce((s, d) => s + d.totalValue, 0);
  const grandTotal = connectionTotal + adhocTotal;
  ...
  // ── Delete Execution (With Automatic Inventory Restoration) ─────────────────
  const executeDelete = async (dispatchToDel) => {
    if (dispatchToDel.type === "connection" && dispatchToDel._jobId) {
      const jobToDelete = jobs.find((j) => j.id === dispatchToDel._jobId);
      if (jobToDelete) {
        // 1. Safely restore all inventory items back to the warehouse
        for (const issuedItem of jobToDelete.items || []) {
          const invItem = inventoryItems.find(
            (i) => i.id === issuedItem.inventoryItemId,
          );
          if (invItem) {
            // Deduct the removed quantity from "stockOut"
            const newStockOut = Math.max(
              0,
              (invItem.stockOut || 0) - (issuedItem.qty || 0),
            );

            // Clean up the specific log entry for this job from the item's history
            const logs = Array.isArray(invItem.issueLog)
              ? [...invItem.issueLog]
              : [];
            let foundIndex = -1;
            // Iterate backwards to find the latest matching log
            for (let i = logs.length - 1; i >= 0; i--) {
              const log = logs[i];
              if (
                log.qty === issuedItem.qty &&
                (log.subscriberId === jobToDelete.subscriberId ||
                  log.subscriberName === jobToDelete.subscriberName)
              ) {
                foundIndex = i;
                break;
              }
            }
            if (foundIndex !== -1) logs.splice(foundIndex, 1);

            // Save the restored item state (updateItem automatically calculates inHand)
        ...
        // 2. Finally, delete the connection job record itself
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatCard
          ...
          label="All Dispatches"
          value={allDispatches.length}
          sub={`PKR ${grandTotal.toLocaleString()} value`}
        />
        <StatCard
          color="blue"
          label="Connections"
          value={connectionDispatches.length}
          sub={`PKR ${connectionTotal.toLocaleString()} value`}
          extra={
            connectionPendingTotal > 0
              ? `PKR ${connectionPendingTotal.toLocaleString()} pending`
              : "All dues cleared ✓"
          }
          extraColor={
            connectionPendingTotal > 0 ? "text-red-500" : "text-green-600"
          }
        />
        <StatCard
          color="amber"
      ...
      <div className="relative">
        ...
        <input
          ...
          value={search}
          onChange={(e) => handleSearch(e.target.value)}
          className="w-full pl-10 pr-9 py-2.5 text-sm border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm"
        />
        {search && (
          <button
            onClick={() => handleSearch("")}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 bg-gray-100 p-1 rounded-md"
          >
            <X size={14} />
          </button>
        )}
        ...
        {pageItems.length === 0 ? (
          ...
          pageItems.map((dispatch, idx) => {
            ...
            return (
              <div
                key={dispatch.id}
                className={`rounded-xl border-2 bg-white overflow-hidden shadow-sm hover:shadow-md transition-all duration-200 ${
                  isConnection ? "border-blue-100" : "border-amber-100"
                }`}
              >
                {/* Header row */}
                <div
                  className={`flex items-start md:items-center justify-between gap-3 px-4 py-3 cursor-pointer select-none group ${
                    isConnection
                      ? "hover:bg-blue-50/40"
                      : "hover:bg-amber-50/40"
                  } transition-colors`}
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    >
                      <span className="text-xs font-black">#{globalNum}</span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <span
                          ...
                          title={
                            isConnection
                              ? "Stock issued to a specific subscriber for a new connection."
                              : "Stock issued for internal use, repairs, or maintenance."
                          }
                        >
                          {isConnection ? (
                            <>
                            ...
                            <>
                              <Hammer size={10} /> Ad-hoc
                            </>
                          )}
                        </span>
                        {dispatch.subscriberName ? (
                          <span className="flex items-center gap-1 text-sm font-bold text-gray-900 truncate">
                            <User
                              size={12}
                              className={
                                isConnection
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                        ...
                        {!isOpen && dispatch.note && (
                          <span className="truncate max-w-xs text-gray-400 italic">
                            · {dispatch.note}
                          </span>
                        )}
                      </div>
                  ...
                  {/* Financials & Actions on the Right */}
                  <div className="flex items-center gap-4 shrink-0">
                      ...
                      {isConnection ? (
                        dispatch.amountPending > 0 ? (
                          <span
                            ...
                            title="Subscriber has an outstanding balance for this issued stock. Clear it from the Subscribers tab."
                          >
                          ...
                          <span
                            ...
                            title="Subscriber has fully paid for this issued stock."
                          >
                    ...
                    {/* Delete Button (Only for Connections) */}
                    {isConnection ? (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="p-2 rounded-lg text-gray-300 hover:text-red-600 hover:bg-red-50 border border-transparent hover:border-red-100 transition-all"
                        title="Delete connection job record"
                      >
                        <Trash2 size={15} />
                      </button>
                    ) : (
                      <div className="w-9" /> /* Spacer to align items */
                    )}
                  </div>
                </div>
                ...
                {isOpen && (
                  <div className="border-t border-gray-100 bg-gray-50/50 p-4">
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm mb-4">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b border-gray-200 bg-gray-50 text-gray-500 text-left uppercase tracking-wider">
                            <th className="px-4 py-2 font-bold">Item Issued</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {items.map((item, i) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-2.5 text-center">
                                <span className="inline-flex items-center gap-1 font-bold text-gray-800 bg-gray-100 px-2 py-0.5 rounded text-[11px]">
                                  {item.qty}{" "}
                    ...
                    {/* Footer Details & Financials */}
                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                      {/* Left: Metadata */}
                      <div className="space-y-2 flex-1 text-xs">
                        {dispatch.technicianName && (
                          <div className="flex items-center gap-2 text-gray-600 bg-white border border-gray-200 px-3 py-2 rounded-lg w-fit">
                        ...
                        {dispatch.note && (
                          <div className="flex items-start gap-2 text-gray-600 bg-white border border-gray-200 px-3 py-2 rounded-lg max-w-sm">
                            <FileText
                              size={13}
                              className="text-gray-400 shrink-0 mt-0.5"
                            />
                            <span className="italic">{dispatch.note}</span>
                          </div>
                        )}
                      </div>
                      ...
                      {isConnection ? (
                        <div className="bg-white border border-blue-100 rounded-xl p-4 min-w-[240px] shadow-sm text-sm">
                          ...
                          {/* Action Hint */}
                          {dispatch.amountPending > 0 && (
                            <div className="mt-3 bg-blue-50 text-blue-700 p-2 rounded-lg text-[10px] leading-tight flex items-start gap-1.5 border border-blue-100">
                              <Info size={12} className="shrink-0 mt-0.5" />
                              <span>
                                Use the <strong>Inventory Dues</strong> filter
                                in the Subscribers tab to log a payment for this
                                job.
                              </span>
                            </div>
                          )}
      ...
      {/* ── Delete Confirm (Password Protected) ─────────────────────────────────── */}
      <ConfirmDialog
        isOpen={!!confirmDelete}
        onClose={() => setConfirmDelete(null)}
        onConfirm={() => executeDelete(confirmDelete)}
        title="Delete Connection Job"
        message={`Are you sure you want to delete this connection job for "${
          confirmDelete?.subscriberName || "Unknown"
        }"? The issued stock (${
          confirmDelete?.items?.length || 0
        } items) will be automatically restored to the warehouse inventory.`}
...
function EmptyState({ hasSearch }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-3 text-center border-2 border-dashed border-gray-200 rounded-2xl">
...
// ─── Stat card ────────────────────────────────────────────────────────────────
function StatCard({ color, label, value, sub, extra, extraColor }) {
  const s =
    {
      gray: {
        wrap: "bg-gray-50 border-gray-200",
  ...
  return (
    <div
      className={`border rounded-xl px-3 py-3.5 text-center flex flex-col justify-center ${s.wrap}`}
    >
      <p
        className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${s.lbl}`}
      >
        {label}
      </p>
      <p className={`text-2xl font-black leading-tight ${s.val}`}>{value}</p>
      ...
      {extra && (
        <p
          className={`text-[10px] mt-0.5 font-semibold ${extraColor || s.lbl}`}
        >
          {extra}
        </p>
      )}
...
function PagBtn({ onClick, disabled, children }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors shadow-sm"
    >
      {children}
    </button>
  );
