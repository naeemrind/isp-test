...
// Helper function to figure out exactly what is pending
function getDuesBreakdown(cycle, customerJobs) {
  if (!cycle)
    return {
      prevPending: 0,
      installPending: 0,
  ...
  const prevBal = cycle.previousBalance || 0;
  const installFee = cycle.breakdown?.installationFee || 0;
  // Package fee might be adjusted if we have previous balance. Default fallback:
  const pkgFee =
    cycle.breakdown?.effectivePkgPrice ??
    cycle.totalAmount - installFee - prevBal;

  let remainingPaid = amountPaid;

  const prevBalPaid = Math.min(remainingPaid, prevBal);
  const prevBalPending = prevBal - prevBalPaid;
  remainingPaid -= prevBalPaid;

  const installPaid = Math.min(remainingPaid, installFee);
  const installPending = installFee - installPaid;
  remainingPaid -= installPaid;

  const pkgPending = Math.max(0, pkgFee - remainingPaid);
  ...
  return {
    prevPending: prevBalPending,
    installPending,
    pkgPending,
    invPending,
    total: prevBalPending + installPending + pkgPending + invPending,
  };
}
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-gray-100 text-gray-600 text-left">
              ...
              <th className="px-3 py-2 font-medium">Pending Dues</th>
          ...
          <tbody>
            {filtered.map((customer, idx) => {
              ...
              const dues = getDuesBreakdown(cycle, customerJobs);
              ...
              const hasInventoryIssued = customerJobs.length > 0;
              ...
              return (
                ...
                >
                  <td className="px-3 py-2 align-top pt-3 text-center text-xs text-gray-400 font-medium w-10">
                  ...
                  <td className="px-3 py-2 align-top pt-3 text-gray-600">
                    {customer.mainArea || "-"}
                  </td>
                      ...
                      <div className="space-y-1.5 min-w-[120px]">
                        {dues.prevPending > 0 && (
                          <div className="flex justify-between items-center text-xs">
                            <span className="text-gray-500 mr-2 text-[10px] uppercase font-bold">
                              Arrears
                            </span>
                            <span className="text-red-600 font-bold">
                              {dues.prevPending.toLocaleString()}
                            </span>
                          </div>
                        )}
                        {dues.installPending > 0 && (
                          <div className="flex justify-between items-center text-xs">
                            <span className="text-orange-500 mr-2 text-[10px] uppercase font-bold">
                              Setup
                            </span>
                            <span className="text-orange-600 font-bold">
                              {dues.installPending.toLocaleString()}
                            </span>
                          </div>
                        )}
                        {dues.pkgPending > 0 && (
                          <div className="flex justify-between items-center text-xs">
                            <span className="text-blue-500 mr-2 text-[10px] uppercase font-bold">
                              Pkg
                            </span>
                            <span className="text-blue-600 font-bold">
                              {dues.pkgPending.toLocaleString()}
                            </span>
                          </div>
                        )}
                        {dues.invPending > 0 && (
                          <div className="flex justify-between items-center text-xs">
                            <span className="text-purple-500 mr-2 text-[10px] uppercase font-bold">
                              Equip
                            </span>
                            <span className="text-purple-600 font-bold">
                              {dues.invPending.toLocaleString()}
                            </span>
                          </div>
                        )}
                        {(dues.prevPending > 0 ? 1 : 0) +
                          (dues.installPending > 0 ? 1 : 0) +
                          (dues.pkgPending > 0 ? 1 : 0) +
                          (dues.invPending > 0 ? 1 : 0) >
                          1 && (
                  ...
                  <td className="px-3 py-2 align-top pt-3">
                    ...
                    {dues.invPending > 0 && (
                      <div className="mt-1.5">
                        <span
                          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700 border border-purple-200 shadow-sm"
                          title={`Equipment dues: PKR ${dues.invPending.toLocaleString()}`}
                        >
                          <Package size={10} /> Equip Due
                        </span>
                      </div>
                    )}
                  </td>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {hasAnyInventory && (
                        <button
                          ...
                          className={
                            hasInventoryIssued
                              ? "flex items-center justify-center w-7 h-7 rounded border bg-white border-blue-200 text-blue-400 hover:bg-blue-50 hover:border-blue-400 hover:text-blue-600 transition-colors"
                              : "flex items-center justify-center w-7 h-7 rounded border bg-white border-amber-200 text-amber-500 hover:bg-amber-50 hover:border-amber-400 hover:text-amber-700 transition-colors"
                          }
                          title={
                            hasInventoryIssued
                              ? "Issue More Inventory to Subscriber"
                              : "Issue Inventory to Subscriber"
                          }
                        >
                          <Package size={14} />
                        </button>
